    ...
    try {
      var res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "user", content: "Parse every race from this HRI race conditions text into a JSON array. Return ONLY a raw JSON array, no markdown, no explanation. Each race object needs: id (unique string like race_001), raceName, venue, date (YYYY-MM-DD), entryDeadline (YYYY-MM-DD or null), discipline (Flat/Hurdle/Chase), surface (Turf/AWT), distanceFurlongs (number), forecastGoing, prizeMoney (number), grade (string or null), sexRestriction (Open/Mares/Fillies/Colts), ageMin (number), ageMax (number or null), ratingMin (number or null), ratingMax (number or null), isMaiden (boolean), isNovice (boolean), isEBF (boolean).\n\nText:\n\n" + pasteText }]
        })
      });
      var data = await res.json();
      var txt = (data.content || []).filter(function(b) { return b.type === "text"; }).map(function(b) { return b.text; }).join("");
