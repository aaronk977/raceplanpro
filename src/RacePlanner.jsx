    ...
    try {
      var res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "user", content: "Parse every race from this HRI or BHA race conditions text into a JSON array. Return ONLY a raw JSON array, no markdown, no explanation. Each race object MUST use these exact field names: id (race_001 etc), raceName, venue, date (YYYY-MM-DD), entryDeadline (YYYY-MM-DD or null), discipline (must be exactly: Flat OR Hurdle OR Chase OR Bumper), surface (Turf or AWT), distanceFurlongs (number), forecastGoing, prizeMoney (number), grade (Grade 1/2/3 or Group 1/2/3 or null), sex (must be exactly: Open OR Mares OR Fillies OR Colts), minAge (number in years e.g. 4), maxAge (number or null), minRating (number or null - lower rating limit), maxRating (number or null - upper rating limit VERY IMPORTANT for handicaps), isMaiden (boolean - true if horse must not have won), isNovice (boolean), isEBF (boolean), raceType (Handicap/Conditions/Maiden/Novice/Listed/Graded/Other). CRITICAL: minRating and maxRating must be exact numbers from the conditions - these determine which horses qualify. For a 0-90 rated handicap: minRating=0 maxRating=90. For rated 95+: minRating=95 maxRating=null. Discipline must never be NH or Jump - always Hurdle or Chase. Sex must never be M/F - always Mares/Fillies/Open/Colts.\n\nText:\n\n" + pasteText }]
        })
      });
      var data = await res.json();
      var txt = (data.content || []).filter(function(b) { return b.type === "text"; }).map(function(b) { return b.text; }).join("");
