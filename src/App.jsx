                ...
                <div key={race.id} style={{ background: C.card, borderRadius: 13, border: "1px solid " + (isSl ? C.gold + "60" : C.border), marginBottom: 12, overflow: "hidden", boxShadow: isSl ? "0 2px 12px " + C.gold + "15" : C.shadow }}>
                  ...
                  <div style={{ padding: "14px 16px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                      ...
                      <span style={{ fontSize: 18, fontWeight: 800, color: C.gold, flexShrink: 0 }}>{"€" + (race.prizeMoney >= 1000 ? Math.round(race.prizeMoney * 0.001) + "k" : race.prizeMoney)}</span>
                    </div>
                      ...
                      <div style={{ marginBottom: 10 }}>
                        <div style={{ display: "flex", gap: 5, marginBottom: 10 }}>
                          {[["HCP", "handicap_edge"], ["Class", "class_fit"], ["Going", "conditions_match"], ["Timing", "timing"], ["Angle", "cuteness"]].map(([label, k2]) => (
                            <div key={k2} style={{ flex: 1, textAlign: "center", padding: "6px 2px", background: (analysis.scores[k2] >= 7 ? C.green : analysis.scores[k2] >= 5 ? C.amber : C.red) + "10", borderRadius: 7, border: "1px solid " + (analysis.scores[k2] >= 7 ? C.green : analysis.scores[k2] >= 5 ? C.amber : C.red) + "25" }}>
                              <div style={{ fontSize: 15, fontWeight: 800, color: analysis.scores[k2] >= 7 ? C.green : analysis.scores[k2] >= 5 ? C.amber : C.red }}>{analysis.scores[k2]}</div>
